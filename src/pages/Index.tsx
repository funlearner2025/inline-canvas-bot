...
export default function Index() {
  ...
  const [showDailyAstro, setShowDailyAstro] = useState(false);
    ...
    <div className="min-h-screen relative text-white overflow-hidden">
      ...
      <div className="relative z-10 min-h-screen" style={{
        ...
        <AnimatePresence mode="wait">
          {!showDailyAstro ? (
            <motion.div key="intro" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col min-h-screen">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 p-6 bg-gradient-to-t from-black/40 to-transparent">
                <motion.div className="relative rounded-2xl overflow-hidden shadow-2xl cursor-pointer bg-gradient-to-br from-indigo-600 to-purple-600 p-6 min-h-[200px] flex flex-col justify-between" onClick={() => setShowDailyAstro(true)} initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.5 }} whileHover={{ scale: 1.03, y: -5 }} whileTap={{ scale: 0.98 }}>
                  ...
                  <div className="relative z-10">
                    ...
                    <h2 className="text-xl sm:text-2xl font-bold mb-2 drop-shadow-lg">Celestial Vibe Today</h2>
                    <p className="text-white/90 text-sm drop-shadow-md">Daily Astro Insights</p>
                  </div>
                  <motion.div className="relative z-10 mt-4 flex items-center gap-2" whileHover={{ x: 5 }} transition={{ duration: 0.2 }}>
            ...
            <motion.div key="dailyastro" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col justify-between min-h-screen px-6 py-8">
              <button onClick={() => { setShowDailyAstro(false); setAstroData(null); setError(null); }} className="self-start mb-4 text-white/60 hover:text-white transition-colors">← Back</button>
